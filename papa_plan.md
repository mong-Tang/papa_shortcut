...
- 편집기에서 추가한 카테고리는 상단 버튼 아래 `추가 카테고리 선택...` 드롭다운에 노출
- 추가 카테고리 선택 시 기존 필터와 동일하게 즉시 목록 필터링
...
- 메인 레이아웃 그리드 행 수 보정(`extra-category-row` 추가 후 리스트 하단 정렬 문제 해결)
- 깨진 한글 UI 문자열 교정 및 인코딩 정책 파일 추가(`.editorconfig`, `.gitattributes`)
...
7. 메인의 `추가 카테고리 선택...`에서 신규 카테고리 선택 가능 여부 확인
8. 카테고리 변경 시 아이템 목록이 항상 상단부터 표시되는지 확인
9. 앱 재실행 후 카테고리/아이템 유지 확인
